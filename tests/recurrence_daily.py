...
import json
import pytest
from utils.recurrence_reuests import create_recurring_task, update_recurring_task, get_recurred_task
# delete_task
...
class TestProofHubAPI:
    ...
    def test_create_recurring_task(self):
        ...
        title = "Automated Task using python script recurring task daily"
        description = "Task created via API automation test using python script recurring task daily."
        ...
        assert response.status_code == 200, "Task not created"
